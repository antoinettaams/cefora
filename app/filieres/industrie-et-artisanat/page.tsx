...
export default function Hero() {
  // Données de la section d'admission
  const scheduleData = [
      ...
      <section className="bg-white py-20 text-center overflow-x-hidden">
        ...
        <div className="flex flex-col items-center gap-6 scale-[0.8] sm:scale-[0.9] md:scale-100 transition-transform duration-300">
          <div className="flex justify-center gap-20">
            {[
              { color: "#FDBA21", text: "Sérigraphie", textColor: "#0F172A" },
              { color: "#FDBA21", text: "Vitrerie", textColor: "#0F172A" },
            ].map((item, index) => (
          ...
          <div className="flex justify-center gap-6">
            {[
              { color: "#F43F3E", text: "Soudure et Construction Métallique", textColor: "#fff" },
              { color: "#1E3AFF", text: "Certificat inclus", textColor: "#fff" },
              { color: "#FDBA21", text: "Durée : 2 ans", textColor: "#0F172A" },
            ].map((item, index) => (
              <div key={index} className="flex items-center gap-2 px-6 py-3 rounded-full font-medium whitespace-nowrap shadow-sm"
                   style={{ backgroundColor: item.color, color: item.textColor }}>
                <span className="w-2 h-2 bg-white rounded-full"></span>
